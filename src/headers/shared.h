...
#define os_calloc(x,y,z) (z) = calloc(x,y);(z)?(void)1:ErrorExit(MEM_ERROR, ARGV0)

#define os_strdup(x,y) (y) = strdup(x);(y)?(void)1:ErrorExit(MEM_ERROR, ARGV0)

#define os_malloc(x,y) (y) = malloc(x);(y)?(void)1:ErrorExit(MEM_ERROR, ARGV0)

#define os_free(x) (x)?free(x):merror("free a null")

#define os_realloc(x,y,z) (z) = realloc(x,y);(z)?(void)1:ErrorExit(MEM_ERROR, ARGV0)

#define os_clearnl(x,p) (p) = strrchr(x, '\n');if(p)*(p) = '\0';
