...
int OS_algorithms_File(const char *fname, const char *prefilter_cmd, alg_output file_output, int mode, syscheck_config syscheck)
{
    size_t n;
    FILE *fp;
    ...
    while(syscheck->alg[al]) {
        if((strncmp(syscheck->alg[al], "md5", 3) == 0 || strncmp(syscheck->alg[al],
                        "MD5", 3) == 0)) {
            c_md5 = 1;
        } else if((strncmp(syscheck->alg[al], "sha1", 4) == 0 || strncmp(syscheck->alg[al],
                        "SHA1", 4) == 0)) {
            c_sha1 = 1;
        } else if((strncmp(syscheck->alg[al], "sha256", 6) == 0 || strncmp(syscheck->alg[al],
                        "SHA256", 6) == 0)) {
